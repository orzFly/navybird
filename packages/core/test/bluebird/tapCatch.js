...
describe("tapCatch", function() {
  ...
  specify(
    "passes through reason after returned promise is fulfilled",
    function() {
      var async = false;
      return rejection()
        .tapCatch(function() {
  ...
  specify("Works with predicates", function() {
    ...
    return Promise.reject(new TypeError())
      .tapCatch(TypeError, function(err) {
        called = true;
  ...
  specify("Does not get called on predicates that don't match", function() {
    var called = false;
    return Promise.reject(new TypeError())
      .tapCatch(ReferenceError, function(a) {
        called = true;
      })
      .then(assert.fail, function(err) {
        assert(called === false);
  ...
  specify("Supports multiple predicates", function() {
    ...
    var promiseA = Promise.reject(new ReferenceError())
      .tapCatch(ReferenceError, TypeError, function(e) {
        assert(e instanceof ReferenceError);
        calledA = true;
      })
      .catch(function() {});

    var promiseB = Promise.reject(new TypeError())
      .tapCatch(ReferenceError, TypeError, function(e) {
    ...
    var promiseC = Promise.reject(new SyntaxError())
      .tapCatch(ReferenceError, TypeError, function(e) {
        calledC = true;
      })
      .catch(function() {});
