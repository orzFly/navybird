...
module.exports = {
  awaitGlobalException: function(fn) {
    function replaceListeners(by) {
      ...
      if (process.title === "browser") {
        ...
        window.onerror = single
          ? function(message, file, line, column, e) {
              return by(e);
            }
          : by[0];
        return [original];
  ...
  onUnhandledFail: function(testFunction) {
    return new Promise(function(resolve, reject) {
      var err = new Error(
        "Reporting handled rejection as unhandled from: " + testFunction
      );
  ...
  onUnhandledSucceed: function(testAgainst, count) {
    return new Promise(function(resolveTest, reject) {
      ...
      Promise.delay(200).then(function() {
        var message =
          "Expected onPossiblyUnhandledRejection to be called " +
          total +
          " times but it was only called " +
          cur +
          " times";
